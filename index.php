...
<html>
    ...
    <body >
        ...
        function createscreen()
        {
            ...
            basic.innerHTML='<input type="image" src="logo/DB.png" onclick="DB();" id="Db" width="80" height="80" /><input type="image" src="logo/building.gif" onclick="building("buil");" id="Building" width="80" height="80"/><br>';
...
var option=document.createElement('option');
		...
		error: function() {
            ...
            function graph(){
                ...
                if(floor!=0)controls.rotateSpeed = 0.0;
				else controls.rotateSpeed = 0.5;
...
counter+=1;
                        if (edges[i].type=="Hallway")var material = new THREE.LineBasicMaterial({color: 0x0000FF, linewidth: 25, fog:true});
                        else var material = new THREE.LineBasicMaterial({color: 0x999966, linewidth: 2, fog:true});
                	var lineg = new THREE.Geometry();
...
alert(counter);
				for(var i = 0; i < nodes.length; i++){
                    if (floor == nodes[i].floor || floor==0){
                        if (nodes[i].type == 'Hallway')var geometry = new THREE.CubeGeometry( 1, 1, 0);
                        else if(nodes[i].type == 'Room')var geometry = new THREE.CubeGeometry( nodes[i].width, nodes[i].height, 0);
                        else   	var geometry = new THREE.CubeGeometry(15, 15, 0);
